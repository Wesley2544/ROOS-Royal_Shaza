  ...
  return (
    <div className="p-5 max-w-2xl space-y-5">
      ...
      <div className="bg-white border border-[#E5E5E5] rounded-[18px] p-6">
        ...
        {pwSuccess && <div className="mb-3 px-3 py-2 bg-green-100 border border-green-200 rounded-xl text-xs text-green-800 font-medium">Password updated successfully.</div>}
        {pwError && <div className="mb-3 px-3 py-2 bg-red-100 border border-red-200 rounded-xl text-xs text-red-800 font-medium">{pwError}</div>}
        <form onSubmit={handlePasswordSubmit} className="space-y-3">
          ...
          <button type="submit" disabled={pwLoading} className="w-full py-2.5 bg-[#FDC700] text-[#0A0A0A] rounded-xl text-sm font-bold hover:brightness-95 disabled:opacity-60 mt-2">
        ...
        {trashLoading ? (
          ...
          <div className="border border-[#E5E5E5] rounded-xl overflow-hidden">
            <div className="grid grid-cols-[1fr_1fr_90px_110px_170px] bg-[#F5F5F5] px-3 py-2">
              {['Name','Username','Role','Deleted','Actions'].map(t => <span key={t} className="text-[10px] font-bold text-gray-500 uppercase tracking-wide">{t}</span>)}
            </div>
            {trash.map((member, idx) => (
              <div key={member.id} className={`grid grid-cols-[1fr_1fr_90px_110px_170px] px-3 py-2.5 items-center text-xs border-t border-[#E5E5E5] ${idx % 2 === 1 ? 'bg-[#F5F5F5]' : ''}`}>
                ...
                <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-bold w-fit ${ROLE_STYLE[member.role]}`}>{member.role.charAt(0).toUpperCase() + member.role.slice(1)}</span>
                <span className="text-gray-400">{new Date(member.deleted_at).toLocaleDateString([], { dateStyle: 'medium' })}</span>
                <div className="flex gap-1.5">
                  <button onClick={() => setRestoreTarget({ id: member.id, name: member.name })} className="px-2.5 py-1 rounded-lg text-[11px] font-bold border border-green-200 bg-green-50 text-green-700 hover:bg-green-100">Restore</button>
                  <button onClick={() => { setPermDeleteTarget({ id: member.id, name: member.name }); setPermDeleteError('') }} className="px-2.5 py-1 rounded-lg text-[11px] font-bold border border-red-300 bg-white text-red-700 hover:bg-red-50">Delete forever</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {restoreTarget && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-[18px] shadow-xl w-full max-w-sm p-6">
            ...
            <p className="text-xs text-gray-500 mb-4">Reappears in Staff Accounts as deactivated. Reactivate it from there.</p>
            <div className="flex gap-2">
              <button onClick={() => setRestoreTarget(null)} className="flex-1 py-2 border border-[#E5E5E5] text-gray-600 rounded-xl text-xs font-bold hover:bg-[#F5F5F5]">Cancel</button>
              <button onClick={handleRestore} disabled={restoreStaff.isPending} className="flex-1 py-2 bg-green-700 text-white rounded-xl text-xs font-bold hover:bg-green-800 disabled:opacity-60">
      ...
      {permDeleteTarget && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-[18px] shadow-xl w-full max-w-sm p-6">
            ...
            <p className="text-xs text-gray-500 mb-4">Cannot be undone. Blocked automatically if this account has served real orders.</p>
            {permDeleteError && <div className="mb-3 px-3 py-2 bg-red-100 border border-red-200 rounded-xl text-xs text-red-800 font-medium">{permDeleteError}</div>}
            <div className="flex gap-2">
              <button onClick={() => setPermDeleteTarget(null)} className="flex-1 py-2 border border-[#E5E5E5] text-gray-600 rounded-xl text-xs font-bold hover:bg-[#F5F5F5]">Cancel</button>
              <button onClick={handlePermanentDelete} disabled={permDelete.isPending} className="flex-1 py-2 bg-red-700 text-white rounded-xl text-xs font-bold hover:bg-red-800 disabled:opacity-60">
