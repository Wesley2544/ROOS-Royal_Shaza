    ...
    <div className="p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setModalOpen(true)} className="px-4 py-2 bg-[#FDC700] text-[#0A0A0A] text-xs font-bold rounded-xl hover:brightness-95">
            + Add staff
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[18px] border border-[#E5E5E5] overflow-hidden">
        ...
        {staff?.length === 0 ? (
          ...
          staff?.map((member, idx) => (
            <div key={member.id} className={`grid grid-cols-[1fr_1fr_100px_90px_180px] px-4 py-3 items-center text-xs border-t border-[#E5E5E5] ${idx % 2 === 1 ? 'bg-[#F5F5F5]' : ''}`}>
              ...
              <div className="flex gap-1.5">
                <button onClick={() => setConfirmTarget({ id: member.id, name: member.name, nextActive: !member.is_active })}
                ...
                {member.role !== 'manager' && (
                  <button onClick={() => setDeleteTarget({ id: member.id, name: member.name })}
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-[18px] shadow-xl w-full max-w-md p-6">
            ...
            {formError && <div className="mb-3 px-3 py-2 bg-red-100 border border-red-200 rounded-xl text-xs text-red-800 font-medium">{formError}</div>}
            <form onSubmit={handleAddStaff} className="space-y-3">
              ...
              <div className="flex gap-2 pt-2">
                <button type="button" onClick={() => setModalOpen(false)} className="flex-1 py-2.5 border border-[#E5E5E5] text-gray-600 rounded-xl text-sm font-bold hover:bg-[#F5F5F5]">Cancel</button>
      ...
      {confirmTarget && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-[18px] shadow-xl w-full max-w-sm p-6">
            <h3 className="text-sm font-bold text-[#0A0A0A] mb-2">{confirmTarget.nextActive ? 'Reactivate' : 'Deactivate'} {confirmTarget.name}?</h3>
            <p className="text-xs text-gray-500 mb-4">
              {confirmTarget.nextActive ? 'They will be able to log in again immediately.' : 'They will be signed out and unable to log in until reactivated. Past orders they handled are not affected.'}
            </p>
            <div className="flex gap-2">
              <button onClick={() => setConfirmTarget(null)} className="flex-1 py-2 border border-[#E5E5E5] text-gray-600 rounded-xl text-xs font-bold hover:bg-[#F5F5F5]">Cancel</button>
              <button onClick={handleConfirmToggle} disabled={toggleActive.isPending}
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-[18px] shadow-xl w-full max-w-sm p-6">
            ...
            <p className="text-xs text-gray-500 mb-4">They'll be signed out immediately. This is reversible — restore anytime from Settings.</p>
            <div className="flex gap-2">
              ...
              <button onClick={handleConfirmDelete} disabled={deleteStaff.isPending} className="flex-1 py-2 bg-red-700 text-white rounded-xl text-xs font-bold hover:bg-red-800 disabled:opacity-60">
