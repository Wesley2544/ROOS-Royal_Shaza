...
import CumulativeChart from '@/components/manager/CumulativeChart'
...
export default function ReportsPage() {
  ...
  function handleExportCSV() {
    ...
    const rows = [
      ...
      ...data.cumulative_revenue.map(p => [p.label, String(p.amount)]),
      [],
      ['Top items', 'Count'],
  ...
  return (
    <div className="p-5">
        ...
        <div className="flex gap-1.5">
          {RANGES.map(r => (
            <button key={r.key} onClick={() => setRange(r.key)}
              className={`px-3 py-1.5 rounded-xl text-xs font-bold border transition-colors ${range === r.key ? 'bg-[#FDC700] text-[#0A0A0A] border-[#FDC700]' : 'border-[#E5E5E5] text-gray-600 hover:bg-[#F5F5F5]'}`}>
              {r.label}
            </button>
          ))}
        </div>
      ...
      <div className="bg-white border border-[#E5E5E5] rounded-[18px] p-4 mb-4">
        <div className="text-sm font-bold text-[#0A0A0A] mb-3">Cumulative revenue</div>
        <CumulativeChart data={data?.cumulative_revenue || []} />
      </div>
          ...
          {(!data?.staff_performance || data.staff_performance.length === 0) ? (
            ...
            <div className="border border-[#E5E5E5] rounded-xl overflow-hidden">
              <div className="grid grid-cols-[1fr_50px_70px_90px] bg-[#0A0A0A] px-3 py-2">
                {['Waiter','Orders','Avg time','Revenue'].map(t => <span key={t} className="text-[10px] font-bold text-white">{t}</span>)}
              </div>
          ...
          <div className="space-y-2.5">
            {data.revenue_by_category.map(cat => (
              <div key={cat.category} className="flex items-center gap-2.5">
                <span className="text-xs text-gray-600 w-24 flex-shrink-0">{cat.category}</span>
                <div className="flex-1 h-2 bg-[#F4F4F5] rounded-full overflow-hidden">
                ...
                <span className="text-xs text-gray-400 w-20 text-right">{formatPrice(cat.amount)}</span>
              </div>
            ))}
          </div>
        )}
