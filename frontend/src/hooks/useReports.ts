...
export interface CumulativePoint { label: string; amount: number }
export interface ReportSummary {
  active_orders: number; tables_occupied: number; avg_wait_minutes: number
  revenue_today: number; total_orders: number; orders_served: number
  ...
  cumulative_revenue: CumulativePoint[]
}
